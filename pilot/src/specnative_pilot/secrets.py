...
def _run_secret_command(command: list[str], *, cwd: Path, missing: str, failure: str, input_text: str | None = None) -> str:
...
def resolve_credentials(config: Config) -> ResolvedCredentials:
    ...
    api_base = _require_string(values.get("api_base"), "api_base", provider) or config.api_base
    if provider in {"sops", "gopass"}:
